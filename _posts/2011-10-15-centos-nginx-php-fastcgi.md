...
author: Stanley
layout: post
guid: http://www.mstz.us/?p=1376
permalink: /centos-nginx-php-fastcgi/
categories:
  - 我的世界
tags:
  - CentOS
...
然后编辑nginx.conf文件，包含sites-enabled目录即可。在`include /etc/nginx/conf.d/*.conf`之后添加。
...
okay，现在开始编辑虚拟主机www.example.com文件的内容。 
    ...
    location ~ \.php$ { try_files $uri =404; if ( $fastcgi_script_name ~ \..*\/.*php ) { return 403; }  
...
c.由于安全问题，需要将/etc/php.ini中的`cgi.fix_pathinfo`设置为0.