...
author: Stanley
layout: post
guid: http://www.mstz.us/?p=1478
permalink: /goagent-error-under-opensuse/
categories:
  - 我的世界
tags:
  - opensuse
...
在运行goagent的时候，会出现下面的几个问题。 
  ...
  1. py-gevent组件 
    ...
    图形界面：yast-软件管理-搜索安装python-gevent 
  2. libnss-tools软件 
  
    `sudo zypper install mozilla-nss-tools`
  
    图形界面：yast-软件管理-搜索安装`mozilla-nss-tools`
  3. 安装根证书失败，显示`certutil: function failed: security library: bad database`.这个错误 
  
    新安装的系统是因为没有初始化证书数据库，所以安装失败，重新初始化即可。</p> 
    > mkdir -p $HOME/.pki/nssdb certutil -d $HOME/.pki/nssdb -N</li> </ol> </blockquote>