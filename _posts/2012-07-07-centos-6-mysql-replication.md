...
author: Stanley
layout: post
guid: http://www.mstz.us/?p=1442
permalink: /centos-6-mysql-replication/
categories:
  - 我的世界
tags:
  - CentOS
...
在主服务器上建立slave用户并授权，192.168.0.199为从服务器的IP。

    mysql> grant replication slave on *.* to 'slave'@'192.168.0.199' identified by '1234';  
    mysql> show master status;  
    ------------------ ---------- -------------- ------------------ | File | Position | Binlog_Do_DB | Binlog_Ignore_DB | ------------------ ---------- -------------- ------------------ | mysql-bin.000025 | 511 | | | ------------------ ---------- -------------- ------------------ 
...
检查运行状态
    ...
    *************************** 1. row ***************************
    ...
    Slave_IO_Running: Yes 此两项一定要为Yes,不然就表示未同步成功 Slave_SQL_Running: Yes  
    Replicate_Do_DB:  
    Replicate_Ignore_DB:  
    Replicate_Do_Table:  
