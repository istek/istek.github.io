...
author: Stanley
layout: post
guid: http://www.mstz.us/?p=1434
permalink: /centos-6-2-ngxin-php-mysql/
categories:
  - 我的世界
tags:
  - CentOS
...
<div id="toc_container" class="toc_white no_bullets">
  ...
  <ul class="toc_list">
    <li>
      <a href="#1">1.使用非官方软件源</a>
    </li>
    <li>
      <a href="#2MYSQL">2.安装MYSQL</a>
    </li>
    <li>
      <a href="#3nginx">3.安装nginx</a>
    </li>
    <li>
      <a href="#4phpphp-fpm">4.安装php和php-fpm</a>
    </li>
    <li>
      <a href="#5nginx">5.配置nginx</a>
    </li>
    <li>
      <a href="#6PHP-FPMunix">6.让PHP-FPM使用unix套接字</a>
    </li>
  </ul>
</div>

### <span id="1">1.使用非官方软件源</span> {#1}
...
同样的，也需要对remi源进行优先级编辑和启用
    ...
    [remi] name=Les RPM de remi pour Enterprise Linux $releasever - $basearch #baseurl=http://rpms.famillecollet.com/enterprise/$releasever/remi/$basearch/ mirrorlist=http://rpms.famillecollet.com/enterprise/$releasever/remi/mirror 
    enabled=1  
    priority=10  
    gpgcheck=1  
...
### <span id="2MYSQL">2.安装MYSQL</span> {#2mysql}
...
### <span id="3nginx">3.安装nginx</span> {#3nginx}
...
![](https://static.howtoforge.com/images/nginx_php5_mysql5_centos_6.2/big/1.png?w=720)

### <span id="4phpphp-fpm">4.安装php和php-fpm</span> {#4phpphpfpm}
...
其中使用的php-x扩展，可以根据自己的需要添加。之后，编辑php.ini文件，修改`cgi.fix_pathinfo`为``。为什么要这么做？请查阅该[文档](http://wiki.nginx.org/Pitfalls)。
...
### <span id="5nginx">5.配置nginx</span> {#5nginx}

关于配置nginx可以查阅官方的两个例子，[例子1](http://wiki.codemongers.com/NginxFullExample)，[例子2](http://wiki.codemongers.com/NginxFullExample2)。

### <span id="6PHP-FPMunix">6.让PHP-FPM使用unix套接字</span> {#6phpfpmunix}

默认情况下，php-fpm使用127.0.0.1:9000监听请求，当然也可以使用unix套接字，这样可以避免TCP的额外流量。编辑`/etc/php-fpm.d/www.conf`文件就可以了。
